...
def get_camera_attributes():
    ...
    if (hasattr(bpy.context.scene, "VisibilityProps")):
...
def set_camera_attributes(camera_attributes):
    ...
    if (isinstance(camera_attributes, str)):
        print("camera_attributes is a string")
        raise Exception("Incorrect deserialization of string")
        
    print("X: " + str(camera_attributes["rotation_euler_x"]))
    obj_camera = bpy.context.scene.camera
    ...
    if (hasattr(bpy.ops, "scene") and hasattr(bpy.ops.scene, "set_visibility") and ("visibility_set" in camera_attributes)):
        visibility_set = camera_attributes["visibility_set"]
        if visibility_set:
            bpy.ops.scene.set_visibility(name=camera_attributes["visibility_set"])
...
bpy.app.timers.register(execute_queued_functions, first_interval=0.1)